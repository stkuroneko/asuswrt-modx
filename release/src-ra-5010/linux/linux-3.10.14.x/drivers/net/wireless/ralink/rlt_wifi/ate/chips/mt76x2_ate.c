...
*/
	...
	/* for "tx_rx_setting" which is hardcoded as 0x202 above */
	pATEInfo->TxAntennaSel = ANT_ALL;
	pATEInfo->RxAntennaSel = ANT_ALL;
	...
	pATEInfo->PreviousChannel = pATEInfo->Channel;
	...
	pATEInfo->TxAntennaSel = TxAntennaSel;
	pATEInfo->RxAntennaSel = RxAntennaSel;
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	if ((pAd->chipCap.tssi_enable) &&
		...
		if(pAd->ate.bSendTSSICmdOnly){
			DBGPRINT(RT_DEBUG_TRACE, ("Send TSSI Inband only \n"));
			return;
		}
...
VOID mt76x2_ate_SendNullFrame(IN PRTMP_ADAPTER pAd)
{
	...
	pATEInfo->HLen = Length;
	pHeader_802_11 = (PHEADER_802_11) (pDMAHeaderBufVA + TXWISize);
	...
	pHeader_802_11->Duration = pAd->CommonCfg.Dsifs;


	ATEWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE,  FALSE, FALSE, FALSE, 
		4, 0, Length /*pATEInfo->TxLength*/, 0, 0, 0, IFS_HTTXOP, FALSE, &TxHTPhyMode);
