		...
		if (IS_VALID_ENTRY(pEntry) && IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("WCID\tMAC Address\t\tmpdu_attempts\tmpdu_retries\t"
					  "minimum latency(sec:usec)\tmaximum latency(sec:usec)\taverage latency(sec:usec) \n"));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%d\t%02X:%02X:%02X:%02X:%02X:%02X\t",
					  pEntry->wcid, PRINT_MAC(pEntry->Addr)));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%10lld\t%10lld\t", pEntry->mpdu_attempts.QuadPart, pEntry->mpdu_retries.QuadPart));
                        MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
                                ("%20ld:%ld\t%20ld:%ld\t%20ld:%ld\n",
                    pEntry->tx_latency_min_tv.tv_sec, pEntry->tx_latency_min_tv.tv_usec,
			        pEntry->tx_latency_max_tv.tv_sec, pEntry->tx_latency_max_tv.tv_usec,
			        pEntry->tx_latency_avg_tv.tv_sec, pEntry->tx_latency_avg_tv.tv_usec));
	...
	case CMD_RTPRIV_IOCTL_ASUSCMD:
		switch (subcmd) {
			...
			case ASUS_SUBCMD_RADIO_STATUS:
			{
				...
				break;
			case ASUS_SUBCMD_RADIO_TEMPERATURE:
			{
			UINT32 temperature = 0;
			RTMP_GET_TEMPERATURE(pAd, &temperature);
			wrq->u.data.length = sizeof(UINT32);
			...
			case ASUS_SUBCMD_GSTAT:
			case ASUS_SUBCMD_GRSSI:
			case ASUS_SUBCMD_GROAM:
			case ASUS_SUBCMD_GETSKUTABLE:
			case ASUS_SUBCMD_GETSKUTABLE_TXBF:
			case ASUS_SUBCMD_CLIQ:
			{
				...
				if (subcmd == ASUS_SUBCMD_GSTAINFO) {
					...
					sprintf(msg+strlen(msg), "HT Operating Mode : 0\n");
					...
					if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry)) && (pEntry->Sst == SST_ASSOC)) {
						...
						if (subcmd == ASUS_SUBCMD_CLIQ) {
							if (!IS_ENTRY_APCLI(pEntry))
								...
								CHAR Rssi_Quality = 0;
								CHAR Rssi = RTMPMaxRssi(pAd, pEntry->RssiSample.AvgRssi[0], pEntry->RssiSample.AvgRssi[1],
															pEntry->RssiSample.AvgRssi[2]);
								if (pEntry->RssiSample.AvgRssi[3] < 0 && pEntry->RssiSample.AvgRssi[3] > Rssi)
									Rssi = pEntry->RssiSample.AvgRssi[3];
								if (Rssi >= -50)
									Rssi_Quality = 100;
